$(document).ready(function() {
    ...
    function loadAccounts(page = 1, query = '') {
      const apiUrl = query ? `http://127.0.0.1:8787/admin/account/list?page=${page}&search=${query}` : `http://127.0.0.1:8787/admin/account/list?page=${page}`;
    ...
    window.updateAccountStatus = function(accountId, status) {
        $.ajax({
        url: `http://127.0.0.1:8787/admin/account/verify`,
        type: 'PUT',
        beforeSend: function(xhr) {
            const token = getToken();
            if (token) {
    ...
    window.deleteAccount = function(accountId) {
      if (confirm('Bạn có chắc chắn muốn xóa tài khoản này không?')) {
        $.ajax({
          url: `http://127.0.0.1:8787/admin/account/delete`,
          type: 'DELETE',
          beforeSend: function(xhr) {
            const token = getToken();
            if (token) {
